...
export const jobMessageSchema = z.discriminatedUnion('kind', [
  ...
  z.object({
    v: z.literal(1),
    kind: z.literal('creative.discover'),
    targetId: z.number().int().positive(),
    source: z.enum(['meta', 'google', 'tiktok']).nullable().default(null),
    reason: z.enum(['discovery', 'tracked', 'viewed', 'manual', 'backfill']),
    taskId: z.string().min(1),
  }),
  z.object({
    v: z.literal(1),
    kind: z.literal('creative.media'),
    adId: z.number().int().positive(),
    variantId: z.number().int().positive(),
    sourceUrl: z.url(),
    mediaType: z.enum(['image', 'video', 'thumbnail']),
    role: z.enum(['primary', 'thumbnail', 'carousel']),
